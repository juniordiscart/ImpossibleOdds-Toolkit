...
namespace ImpossibleOdds.Serialization.Processors
{
    public abstract class AbstractCustomObjectProcessor : ISerializationProcessor, IDeserializationProcessor
    {
        ...
        /// <param name="data">The data structure into which the target will be serialized.</param>
        protected void InvokeOnSerializationCallback(object target, object data)
        {
            if (SupportsSerializationCallbacks)
            {
                InvokeCallback(target, data, CallbackFeature.OnSerializationAttribute);
            }
        }
        ...
        /// </summary>
        /// <param name="target">Target object to notify.</param>
        /// <param name="data">The data structure into which the target has been serialized.</param>
        protected void InvokeOnSerializedCallback(object target, object data)
        {
            if (SupportsSerializationCallbacks)
            {
                InvokeCallback(target, data, CallbackFeature.OnSerializedAttribute);
            }
        }
        ...
        /// </summary>
        /// <param name="target">Target object to notify.</param>
        /// <param name="data">The data structure from which will be deserialized.</param>
        protected void InvokeOnDeserializationCallback(object target, object data)
        {
            if (SupportsSerializationCallbacks)
            {
                InvokeCallback(target, data, CallbackFeature.OnDeserializationAttribute);
            }
        }
        ...
        /// </summary>
        /// <param name="target">Target object to notify.</param>
        /// <param name="data">The data structure from which was deserialized.</param>
        protected void InvokeOnDeserializedCallback(object target, object data)
        {
            if (SupportsSerializationCallbacks)
            {
                InvokeCallback(target, data, CallbackFeature.OnDeserializedAttribute);
            }
        }

        private void InvokeCallback(object target, object data, Type callbackAttributeType)
        {
            ...
            Type processorType = GetType();
            Type targetType = target.GetType();
            Type dataType = data.GetType();
            ISerializationCallback[] callbacks = SerializationUtilities.GetTypeMap(targetType).GetSerializationCallbackMethods(callbackAttributeType);
            ...
            foreach (ISerializationCallback callback in callbacks)
            {
                if (callback.Parameters.Length > 0)
                {
                    ...
                    for (int i = 0; i < callbackParameters.Length; ++i)
                    {
                        ...
                        callbackParams[i] =
                            parameterType.IsAssignableFrom(processorType) ? this :
                            parameterType.IsAssignableFrom(dataType) ? data :
                            SerializationUtilities.GetDefaultValue(parameterType);
                    }

                    callback.Method.Invoke(target, callbackParams);
